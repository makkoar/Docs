...
| `minecraft_version_range`   | Диапазон версий Minecraft, который может использовать этот мод, в формате [диапазона версий Maven](https://maven.apache.org/enforcer/enforcer-rules/versionRanges.html). Обратите внимание, что [снапшоты, пре-релизы и релиз-кандидаты](./Versioning.md#minecraft) не гарантированно сортируются правильно, так как они не следуют семантике версионирования Maven. | `minecraft_version_range=[1.20.6,1.21)`   |
| `neo_version`               | Версия NeoForge, для которой вы создаете мод. Должна совпадать с `minecraft_version`. См. [Версионирование NeoForge](./Versioning.md#neoforge) для получения дополнительной информации о том, как работает версионирование NeoForge. | `neo_version=20.6.62`                     |
...
| `mod_version`               | Версия вашего мода, отображаемая в списке модов. См. [страницу о версионировании](./Versioning.md) для получения дополнительной информации.                                                                                           | `mod_version=1.0`                       |
...
ID мода (Mod ID) — это основной способ отличить ваш мод от других. Он используется в самых разных местах, в том числе как пространство имен для [реестров](../Concepts/Registries.md#deferredregister) вашего мода и как пространство имен для ваших [пакетов ресурсов и данных](../../resources/). Наличие двух модов с одинаковым ID приведет к невозможности загрузки игры.
...
ID группы должен быть установлен в ваш пакет верхнего уровня. См. [Организация пакетов](./Structuring%20Your%20Mod.md#packaging) для получения дополнительной информации.
...
| `version`        | string  | `"1"`                | Версия мода, предпочтительно в [вариации версионирования Maven](./Versioning.md). Если установлено значение `${file.jarVersion}`, оно будет заменено значением свойства `Implementation-Version` в манифесте JAR (отображается как `0.0NONE` в среде разработки). | `version="1.20.2-1.0.0"`                    |
...
| `side`        | string  | `"BOTH"`     | [Физическая сторона](../Concepts/Sides.md), на которой должна присутствовать зависимость: `"CLIENT"`, `"SERVER"` или `"BOTH"`.                                                                                                                           | `side="CLIENT"`          |
...
| :--------------- | :----------------------------------------------------------------------------------- |
| `IEventBus`      | [Шина событий, специфичная для мода](../Concepts/Events.md#шины-событий) (необходима для регистрации, событий и т.д.) |
...
| `Dist`           | [Физическая сторона](../Concepts/Sides.md), на которой загружается этот мод             |
...
По умолчанию аннотация `@Mod` загружается на обеих [сторонах](../Concepts/Sides.md). Это можно изменить, указав параметр `dist`:
